...
import com.ccut.download.db.DownloadDbManager;
...
public class DownloadManager {
     ...
     * 执行队列
     */
    private LinkedList<com.ccut.download.DownloadTask> mDownloadingTaskList;
    /**
     * 最大同时下载数
     */
     ...
     * 等待队列
     */
    private LinkedList<com.ccut.download.DownloadTask> mWaitTaskList;
    /**
     * 暂停队列
     */
    private LinkedList<com.ccut.download.DownloadTask> mPauseTaskList;
    /**
     * 下载任务map
     */
    private HashMap<String, com.ccut.download.DownloadTask> mTaskMap;
    /**
     * 下载线程池
     */
    ...
    private DownloadManager() {
        mDownloadingTaskList = new LinkedList<com.ccut.download.DownloadTask>();
        mWaitTaskList = new LinkedList<com.ccut.download.DownloadTask>();
    ...
    public void addTask(DownloadInfo info) {
        ...
        // 8. 执行下一个下载任务
        com.ccut.download.DownloadTask task = new com.ccut.download.DownloadTask(info);
        if(!mTaskMap.containsKey(task.mId)) {
            task.mState = com.ccut.download.DownloadTask.STATE_WAIT;
            task.onWait();
    ...
    public void stopTask(com.ccut.download.DownloadTask task) {
        // 1. 判断下载任务的状态
        ...
        // TODO 4. 更改数据库信息
        switch (task.mState) {
            case com.ccut.download.DownloadTask.STATE_WAIT:
                mWaitTaskList.remove(task);
                break;
            case com.ccut.download.DownloadTask.STATE_DOWNLOADING:
        ...
        task.mState = com.ccut.download.DownloadTask.STATE_STOP;
        task.stop();
        // 注意 addLast
    ...
    public void removeTask(com.ccut.download.DownloadTask task) {
        // 1. 判断下载任务状态
        ...
        switch (task.mState) {
            ...
            case com.ccut.download.DownloadTask.STATE_DOWNLOADING:
                ...
                mDownloadingTaskList.remove(task);
                break;
            case com.ccut.download.DownloadTask.STATE_STOP:
                mWaitTaskList.remove(task);
                break;
            default:
                return;
    ...
    public void restartTask(com.ccut.download.DownloadTask task) {
        // 1. 判断下载任务状态
        ...
        switch (task.mState) {
            case com.ccut.download.DownloadTask.STATE_STOP:
                task.mState = com.ccut.download.DownloadTask.STATE_DOWNLOADING;
                task.restart();
                mWaitTaskList.add(task);
                break;
            default:
     ...
     * @param task
     */
    public void finishTask(com.ccut.download.DownloadTask task) {
        // 1. 从任务map中移除
        // 2. 从执行队列中移除
        // 3. 从数据库中移除
        // 4. 执行下一个任务
    ...
    private void executeNextTask() {
        ...
        if(mWaitTaskList.size() > 0) {
            com.ccut.download.DownloadTask task = mWaitTaskList.getFirst();
